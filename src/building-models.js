...
import { buildingModelPlacement, offsetLngLat, placedBoxCorners } from "./building-footprint.js";
...
export function createBuildingModels({ map, getBuildings, onReplacedChange, config = BUILDING_MODELS }) {
  let entries = []; // [{ renderId, feature, placement, url }]
  ...
  // Stretch of the GLB's footprint box to the building (1 = modelled to size).
  function stretchOf(entry) {
    const load = loads.get(entry.url);
    if (!load?.fitBox) return null;
    const { fit } = entry.placement;
  ...
  function compute() {
    ...
    for (const feature of getBuildings()?.features || []) {
      ...
      entries.push({ renderId: String(feature.properties.render_id ?? feature.properties.id), feature, placement, url: publicAssetUrl(placement.model) });
    }
    if (entries.length) {
      console.info(`${TAG} ${entries.length} building${entries.length > 1 ? "s" : ""} drawn from GLB models:\n${entries.map(({ feature, placement: p }) => (
  ...
  function debugData() {
    const features = [];
    for (const { feature, placement: p } of entries) {
      const ring = p.footprint.corners.concat([p.footprint.corners[0]]);
      const box = placedBoxCorners({ anchor: p.anchor, rotation: p.rotation, scale: p.size, minX: -p.fit[0] / 2, maxX: p.fit[0] / 2, minZ: -p.fit[2] / 2, maxZ: p.fit[2] / 2 });
      ...
      features.push(
        ...
        { type: "Feature", properties: { kind: "model" }, geometry: { type: "LineString", coordinates: box.concat([box[0]]) } },
  ...
  const api = {
    ...
    placements: () => entries.map(({ renderId, feature, placement: p }) => ({
      type: "Feature",
